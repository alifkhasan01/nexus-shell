...
Singleton {
    ...
    property QtObject options: QtObject {
        ...
        property QtObject position: QtObject {
            property int topMargin: root.readInt("position.topMargin", 70)
        }
    ...
    // Baca config dari ~/.config/quickshell/overview/config.json
    // (lokasi yang sama dengan config.json di folder overview yang sudah ada)
    Process {
        id: loadUserConfig
        command: [
            "sh",
