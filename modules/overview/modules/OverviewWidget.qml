...
Item {
    ...
    readonly property var visibleSpecialWorkspaces: {
        ...
        for (const configured of configuredSpecialWorkspaces ?? []) pushUnique(configured);
    ...
    function getVisibleRowPosition(rowIndex) {
        ...
        if (!Config.options.overview.hideEmptyRows || !(rowsWithContent instanceof Set)) return rowIndex;
        let visibleRow = 0;
    ...
    function specialWorkspaceLabel(name) {
        const raw = `${name ?? ""}`.trim();
        if (raw.length === 0) return "Special";
    ...
    function workspaceHasWindows(workspaceId) {
        ...
        for (const addr in windowByAddress) {
            const win = windowByAddress[addr];
            if (root.isSpecialWorkspace(win)) continue;
            if ((win?.workspace?.id ?? -1) === workspaceId) return true;
        }
        return false;
    }
    ...
    function specialWorkspaceGeometry(name, monitorId) {
        ...
        for (const addr in windowByAddress) {
            ...
            const atX = win?.at?.[0], atY = win?.at?.[1];
            const width = win?.size?.[0], height = win?.size?.[1];
    ...
    Rectangle {
        ...
        Rectangle {
            ...
            radius: Math.max(parent.radius - 1, 0); color: "transparent"
            border.width: 1
        ...
        ColumnLayout {
            ...
            Item { visible: root.showSpecialWorkspaces; implicitWidth: 1; implicitHeight: root.specialStripGap }
            Item {
                ...
                delegate: OverviewWindow {
                    ...
                    MouseArea {
                        ...
                        onReleased: {
                            ...
                            window.pressed = false; window.Drag.active = false
                            root.draggingFromWorkspace = -1; root.draggingTargetWorkspace = -1; root.draggingTargetSpecialWorkspace = ""
                            if (targetSpecialWorkspace === root.createSpecialWorkspaceTarget) {
                                ...
                                window.x = window.initX; window.y = window.initY
                            }
                        }
                        onClicked: (event) => {
        ...
        Item { id: specialWindowDragLayer; anchors.fill: parent; z: root.windowDraggingZ + 1 }
    }
}
